    ...
    /**/&&(CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists=='n'))/****/){//Immediate integer value
     ...
     if(halfBytes==3 && CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists=='y'){
      ...
      if(halfBytes==5){/*If format 4*/
        ...
        modificationRecord += getStringBeforeComma(operand);
        modificationRecord += '\n';
...
void writeDRecord(){
    ...
    for(int i=0;i<length;i++){
      ...
      //mine
        int count;
        splitString(test,',',var_array,count);
      //mine end
  ...
  while(opcode!="=X'05'"){
    ...
    if(!isComment){
      ...
      if(OPTAB[getRealOpcode(opcode)].exists=='y'){
        ...
        int sign=checkSign(operand);
        int count22;
        string operands_string[2];
        
        if(sign==1){
          
          splitString(operand,'+',operands_string,count22);
          mRecord1="M^"+intToStringHex(address)+string("^06+")+operands_string[0];
          mRecord2="M^"+intToStringHex(address)+string("^06+")+operands_string[1];
        
          //writeToFile(objectFile,mRecord1);
          //writeToFile(objectFile,mRecord2);
          }
...
if(!isComment){
  ...
  //writeToFile(objectFile,"HIHIHIHIHIHIIH",false);//Write modification record
  if(flag==1){
    writeToFile(objectFile,mRecord1);
    writeToFile(objectFile,mRecord2);
    flag--;
...
writeData = "\n********************object program for "+ label+" **************";
  ...
  //Do Nothing
    ...
    if(label=="RDREC"){
      writeData= "R^"+var_array[0]+var_array[2]+var_array[1];
      writeToFile(objectFile,writeData);
    }
    else if(label=="WRREC"){
      writeData= "R^"+var_array[0]+var_array[2];
      writeToFile(objectFile,writeData);
    }
...
/*TODO
1)LTORG with *writeToFile(objectFile,writeData);
2)EQU
3)Expressions
*/
...
int main(){
  ...
  string testFile=fileName;
   ifstream source; //pointer inside the file
   source.open(testFile);
  
  
  // Check if the file is empty
    source.seekg(0, ios::end); // Move to the end of the file
    if (source.tellg() < 10) { // Check the position, if 0 then file is empty
        cout << "File not complete! No changes made to other files" << endl;
        
        //writeToFile(intermediateFile," ");
